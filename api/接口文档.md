> **说明**：以下接口通过微信支付开放平台调用。接口参数中 member_id 对应服务商会员号，owner_id 对应服务商号（sp_mchid），batch_code 对应 stock_id（批次号），系统实现跨服务商号为指定可用商户（特约商户）发放预充值代金券。
...
| -------------- | --------------- | ---- | ------------- | ---------------------------- |
| member_id      | 会员号             | M    | String(10-20) | AAA200154561278（拉卡拉提供前缀信息（即服务商会员号））   |
| activity_id    | 活动号             | M    | String(10)    | 1100000086（拉卡拉提供）            |
| owner_id       | 服务商号（原业主Id）            | M    | String(8)     | 80000007（拉卡拉提供）              |
...
| 字段名称            | 约束  | 字段类型   | 长度  | 字段描述         | 取值说明                                 |
| --------------- | --- | ------ | --- | ------------ | ------------------------------------ |
| member_id       | M   | String | 12  | 会员ID         | AAA200154561278（拉卡拉提供前缀信息（即服务商会员号））           |
| owner_id        | M   | String | 8   | 服务商号（原业主ID）         | 80000007                             |
...
| 字段名称         | 内容    | 是否必输 | 最大长度       | 示例值                |
| ------------ | ----- | ---- | ---------- | ------------------ |
| owner_id     | 服务商号（原业主Id）  | M    | String(8)  | 80000007           |
| mer_order_id | 商户订单号 | M    | String(64) | 20211203092301     |
